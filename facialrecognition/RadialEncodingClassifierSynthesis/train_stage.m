...
%% local decision in train stage.
local_classifiers_output = zeros(class_num, block_num * 3, train_file_num);
for train_file_idx = 1 : train_file_num
    min_distance = zeros(class_num, 1);
    for local_block_idx = 1 : block_num * 3    
        min_distance(1 : class_num) = realmax;
        min_distance(class_label(train_file_idx)) = 0;
        
        for train_file_idx_1 = 1 : train_file_num
            fld_projected1 = fld_projected(:, :, local_block_idx);
            distance = norm(fld_projected1(train_file_idx, :) - fld_projected1(train_file_idx_1, :));
            min_distance(class_label(train_file_idx_1)) = min(min_distance(class_label(train_file_idx_1)), distance);
        end
        
        sum = 0;
        for class_idx = 1 : class_num
            sum = sum + 1 / (1 + min_distance(class_idx));
        end
        
        for class_idx = 1 : class_num
            local_classifiers_output(class_idx, local_block_idx, train_file_idx) = 1 / (1 + min_distance(class_idx)) / sum;
        end
    end
end
...
for train_file_idx = 1 : train_file_num
    intermediate_feature(:, train_file_idx) = reshape(local_classifiers_output(:, :, train_file_idx), class_num * block_num * 3, 1);
end


%% apply PCA to project the intermediate feature matrice on to
% low_dimensional subspace.
disp('PCA2 processing...');
pca2_input_samples = intermediate_feature';
[eig_vec, ~] = princomp(pca2_input_samples, 'econ');
pca_matrix2 = eig_vec(:, 1 : train_file_num - class_num);
for eig_vec_idx = 1 : train_file_num - class_num
