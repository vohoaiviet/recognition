function [recogResult]= GaborFR(trainImagePath, testImagePath, imageLable)
...
gaborFilters = GenGaborFilter;
 ...
 %trainImg = zeros(90 * 24, trainImageNum);
 ...
 for i = 1 : trainImageNum
     ...
     %img = GaborFilter(img(1:200, :), 200, 180);
     img = FeatureExtract(img(1:200, :), 200, 180, gaborFilters, ratio);
     trainImg(:, i) = img(:);
 end
 %imshow(img);
 ...
 [COEFF, SCORE] = princomp(trainImg, 'econ'); %econ  p is much larger than n
 ...
 %testImg = zeros(90 * 24, testImageNum);
...
fid = fopen('recognition_meangabor_1_4.txt', 'w');
